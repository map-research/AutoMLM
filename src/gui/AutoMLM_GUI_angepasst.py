...
class AutoMLMApp(ctk.CTk):
    ...
    def _show_models_overview_page(self):
        ...
        self.models_search_var = tk.StringVar(value="")
        ...
        self.models_search_entry = ctk.CTkEntry(
            filters,
        ...
        self.models_search_entry.grid(row=0, column=1, sticky="ew", padx=(0, 44), pady=10)
        ...
        # The outer frame creates one clean, continuous border around the
        # complete dropdown, including the arrow button on the right.
        type_filter_border = ctk.CTkFrame(
            filters,
            width=190,
            height=44,
            corner_radius=7,
        ...
        type_filter_border.grid_propagate(False)
        type_filter_border.grid_columnconfigure(0, weight=1)
        type_filter_border.grid_rowconfigure(0, weight=1)

        self.models_type_filter = ctk.CTkComboBox(
            type_filter_border,
        ...
        self.models_type_filter.grid(row=0, column=0, sticky="nsew", padx=1, pady=1)
    ...
    def _store_loaded_model(self, extension: str):
        ...
        last_modified = self._format_file_modified_time(self.current_file_path)
    ...
    def _current_timestamp_text(self) -> str:
    ...
    def _draw_models_status_cell(
        ...
        badge_bg = self.colors["danger_soft"] if failed else self.colors["success_soft"]
        badge_fg = self.colors["danger"] if failed else self.colors["success"]
        badge_border = "#FCA5A5" if failed else "#86EFAC"
        badge_width = min(width - 24, max(118, len(status) * 7 + 28))
        badge_x = x + (width - badge_width) / 2
        badge_y = y + 8
        badge = self.models_canvas.create_rectangle(
            badge_x,
            badge_y,
            badge_x + badge_width,
            badge_y + height - 16,
    ...
    def _render_overview_details(self, loaded: Optional[LoadedModel] = None):
        ...
        meta.grid(row=0, column=0, sticky="nsew", padx=(0, 14), pady=(0, 12))
        ...
        metrics.grid(row=0, column=1, sticky="nsew", pady=(0, 12))
        metric_colors = {
            "Classes": "#2563EB",
            "Attributes": "#7C3AED",
            "Enumerations": "#F59E0B",
        ...
        # Keep the actions directly below the Objects and Slots cards.
        # Columns 4 and 5 correspond to those two metric cards above.
        actions = ctk.CTkFrame(metrics, fg_color="transparent")
        actions.grid(
            row=1,
            column=4,
            columnspan=2,
    ...
    def _overview_metric_card(
        ...
        muted_color = "#667085" if value == 0 else color
        value_color = "#667085" if value == 0 else color
        icon_map = {
            "Classes": "■",
            "Attributes": "≡",
            "Enumerations": "◇",
        ...
        ctk.CTkLabel(
            ...
            text_color=self.colors["muted"],
            font=ctk.CTkFont(family="Segoe UI", size=12, weight="bold"),
        ).place(relx=0.5, y=28, anchor="center")

        ctk.CTkLabel(
            card,
            text=icon_map.get(label, "■"),
            text_color=muted_color,
            font=ctk.CTkFont(family="Segoe UI Symbol", size=27, weight="bold"),
        ).place(relx=0.34, y=88, anchor="center")
        ...
        ).place(relx=0.62, y=88, anchor="center")
