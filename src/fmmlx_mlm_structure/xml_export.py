...
def exportClass(mlmObject : FmmlxObject, root):
    ...
    for attr in mlmObject.attr_list:
        attribute = ET.SubElement(model, 'addAttribute', level=str(attr.inst_level), multiplicity='Seq{1,1,true,false}', name=attr.name, package=projectName, type=attr.attr_type)
    ...
    for slot in mlmObject.slot_list:
        ...
        slot = ET.SubElement(model, 'changeSlotValue', package = projectName, slotName = slot.attribute.name, valueToBeParsed=str(slot.value))
         # this attr has to be set separetly because of the keyword class and cannot be used in the prior operation
...
def exportEnum(root, enumType: FmmlxEnumType):
    model = root.find('Model')
    addEnum = ET.SubElement(model, 'addEnumeration', name=enumType.name)
    for value in enumType.enum_values:
        addEnumValue = ET.SubElement(model, 'addEnumerationValue', enum_name=enumType.name, enum_value_name=str(value))
